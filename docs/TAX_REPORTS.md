...
Tax reports are compliance documents automatically generated by B2BRouter when you create and send invoices. They contain structured invoice data formatted according to local tax authority requirements.
...
Tax reports are automatically generated when:
1. You create an invoice with `send_after_import: true`
2. You explicitly send an existing invoice using `$client->invoices->send($invoiceId)`
3. Your account has tax reporting configured for your jurisdiction
